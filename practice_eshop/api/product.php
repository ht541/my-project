...
class  Product {
  ...
  public function remove($row){
     ...
     if(!is_numeric($id))
     $this->verdict();
     $sql = $this->db->prepare('delete from product where id=:id')->execute(['id'=>$id]);
  }
  public function update($row){
     ...
     $this->db->prepare('update product set title=:title,price=:price,cat_id=:cat_id,stock=:stock where id=:id')->execute($merged);
  }
